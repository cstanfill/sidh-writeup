...
{ // Testing key exchange
    ...
    {
        Status = KeyGeneration_A(PrivateKeyA, PublicKeyA, CurveIsogeny, false);                     // Get some value as Alice's secret key and compute Alice's public key
        if (Status != CRYPTO_SUCCESS) {                                                  
            goto cleanup;
        } 
...
{ // Benchmarking key exchange
    ...
    {
        cycles1 = cpucycles();
        Status = KeyGeneration_A(PrivateKeyA, PublicKeyA, CurveIsogeny, false);
        if (Status != CRYPTO_SUCCESS) {                                                  
            passed = false;
            break;
        }    
