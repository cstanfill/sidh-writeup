...
CRYPTO_STATUS make_master(char* client_pub, char* server_priv) {
    kex_data bob, alice;
    PCurveIsogenyStruct CurveIsogeny;
    RETURN_IF_ERROR(make_curve(&CurveIsogeny));
    RETURN_IF_ERROR(alloc_kex_data(&bob, CurveIsogeny));
    ...
    unsigned int i;
    for (i = 0; i < 48; ++i) {
        sscanf(server_priv + 2*i, "%02hhx", alice.priv + i);
    }
    for (i = 0; i < 576; ++i) {
        sscanf(client_pub + 2*i, "%02hhx", bob.pub + i);
    }

    RETURN_IF_ERROR(SecretAgreement_A(alice.priv, bob.pub, bob.shared, true, CurveIsogeny));
    for (i = 0; i < 24 * 8; ++i) {
        printf("%02hhX", bob.shared[i]);
    }
    /* blab_digits(true, (digit_t*)bob.shared, 24); */
    printf("\n");
    return CRYPTO_SUCCESS;
}
...
int main(int argc, char** argv) {
    ...
    CRYPTO_STATUS final = make_master(argv[1], argv[2]);
